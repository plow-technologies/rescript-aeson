...
let tuple7 = (first, second, third, fourth, fifth, sixth, seventh, json) =>
  if Js.Array.isArray(json) {
    ...
    if length == 7 {
      ...
      \"@@"(raise, DecodeError(j`Expected array of length 7, got array of length $length`))
    }
  } else {
    \"@@"(raise, DecodeError("Expected array, got " ++ Js.Json.stringify(json)))
  }
...
let tuple8 = (first, second, third, fourth, fifth, sixth, seventh, eighth, json) =>
  if Js.Array.isArray(json) {
    ...
    if length == 8 {
      ...
      \"@@"(raise, DecodeError(j`Expected array of length 8, got array of length $length`))
    }
  } else {
    \"@@"(raise, DecodeError("Expected array, got " ++ Js.Json.stringify(json)))
  }
...
let tuple9 = (first, second, third, fourth, fifth, sixth, seventh, eighth, ninth, json) =>
  if Js.Array.isArray(json) {
    ...
    if length == 9 {
      ...
      \"@@"(raise, DecodeError(j`Expected array of length 9, got array of length $length`))
    }
  } else {
    \"@@"(raise, DecodeError("Expected array, got " ++ Js.Json.stringify(json)))
  }
...
let tuple10 = (first, second, third, fourth, fifth, sixth, seventh, eighth, ninth, tenth, json) =>
  if Js.Array.isArray(json) {
    ...
    if length == 10 {
      ...
      \"@@"(raise, DecodeError(j`Expected array of length 10, got array of length $length`))
    }
  } else {
    \"@@"(raise, DecodeError("Expected array, got " ++ Js.Json.stringify(json)))
  }
...
let singleEnumerator = (a, json) =>
  if Js.Array.isArray(json) {
    ...
    if length == 0 {
      ...
      \"@@"(raise, DecodeError(j`Expected array of length 0, got array of length $length`))
    }
  } else {
    \"@@"(raise, DecodeError("Expected array, got " ++ Js.Json.stringify(json)))
  }
...
let has_some = mas => {
  let count = Array.fold_left((acc, (x, _y)) =>
    switch x {
    | None => acc + 1
    | Some(_a) => acc
    }
...
let beltMap = (decodeKey, decodeValue, ~id, json) =>
  switch array(pair(decodeKey, decodeValue), json) {
  ...
  | exception DecodeError(_) =>
      ...
      let entries = Js.Array.map(((k, v)) => {
        ...
        | exception DecodeError(err) =>
          ...
          | Some(key) =>
            ...
            | key => key
            | exception DecodeError(_) => \"@@"(raise, DecodeError(j`Object key must be a string`))
            }
    ...
    | exception DecodeError(_) =>
      \"@@"(
        raise,
        DecodeError(j`Expected an array of tuples or dictionary of object with string key`),
      )
    }
  }
...
let beltMapInt = (decodeValue, json) =>
  switch dict(decodeValue, json) {
  | decoded_dict =>
    let arr = Array.map(((k, v)) => (Belt.Int.fromString(k), v), Js.Dict.entries(decoded_dict))
    if has_some(arr) {
      \"@@"(raise, DecodeError(j`Unexpectedly received non-integer as key`))
    } else {
      Belt.Map.Int.fromArray(
        Array.map(((k, v)) => (int_of_string(k), v), Js.Dict.entries(decoded_dict)),
      )
    }
  | exception DecodeError(_) =>
    \"@@"(raise, DecodeError(j`Expected an associative array with keys as strings`))
  }
...
let rec oneOf = (decoders, json) =>
  ...
  | list{} =>
    ...
    \"@@"(raise, DecodeError(j`Expected oneOf $length, got ` ++ Js.Json.stringify(json)))
  | list{decode, ...rest} =>
    switch decode(json) {
    | v => v
...
let tryEither = (a, b) => oneOf(list{a, b})
